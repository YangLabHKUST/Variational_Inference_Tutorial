...
def initializeParams(Y, K, singleSigma=False, makePlot=False, random_state=None):
...
def _fit_classic_model(Y, K, singleSigma=False, Y_test=None, random_state=None):
	t0 = time.time()
	Y = deepcopy(Y)
	Y_is_zero = np.array(np.abs(Y) < 1e-6) + 0.0
	Y2 = Y ** 2
	...
	A, mus, sigmas, decay_coef = initializeParams(Y, K, singleSigma=singleSigma, random_state=random_state)
