...
def fit_temporal_model(
    ...
    """Notebook-compatible wrapper around TemporalGP.fit."""
    gp = TemporalGP(model_name=model_name, prior_type=prior_type, guide_type=guide_type)
    losses = gp.fit(
        Y,
        time_index,
        time_kernel,
        ...
        num_steps=num_steps,
        lr=lr,
        clear_param_store=clear_param_store,
        print_every=print_every,
        device=device,
    )
    return losses, dict(gp.config)


def temporal_gp_model(Y, time_index, time_kernel, library_size=None, **kwargs):
    return TemporalGP(model_name="GP_MF", **kwargs).model(
        Y, time_index, time_kernel, library_size
    )


def temporal_gp_guide(Y, time_index, time_kernel, library_size=None, **kwargs):
    return TemporalGP(model_name="GP_MF", **kwargs).guide(
        Y, time_index, time_kernel, library_size
    )
